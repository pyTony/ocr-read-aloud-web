...
from ocr_read_aloud.ad_pages import (
    find_ad_skip,
    folio_of,
    looks_like_ad,
    looks_like_form_ad,
    ...
    *,
    ...
    for ln in raw[1:]:
        ...
        same_col = lines_share_column(prev, ln)
                # Lines with identical bbox are synthetic proof paragraphs
        # (they all carry the full-page rect). Never glue them into one chunk.
        same_bbox = (
            prev.left == ln.left
            and prev.top == ln.top
            and prev.width == ln.width
            and prev.height == ln.height
        )
        if (
            same_col
...
class PageUnit:
    ...
    def speakable(self) -> list[OcrLine]:
        ...
        # Join single-word ALL-CAPS runs (theme words on covers)
                # Join single-word ALL-CAPS runs (theme words on covers)
        chunks = [
            (
                OcrLine(
                    text=join_theme_words(c.text),
...
class PlaybackController:
    ...
    def printed_folio_of(self, page: Any) -> int | None:
        """Return printed page number (folio) of a page, taking offset into account."""
        if page is None:
            return None
        import re
        m = re.search(r"(\d+)", getattr(page, "label", "") or "")
        if m:
            printed = int(m.group(1)) - self._page_offset
    ...
    def _lines_from_proof_text(self, page: PageUnit, new_text: str) -> list[OcrLine]:
        ...
        for para in paras:
            ...
            if matched_lines:
                ...
                x0 = min(ln.left for ln in matched_lines)
                y0 = min(ln.top for ln in matched_lines)
                x1 = max(ln.left + ln.width for ln in matched_lines)
                y1 = max(ln.top + ln.height for ln in matched_lines)
                out.append(
    ...
    def _find_page_target(
        ...
        """
        Find a loaded page by printed folio (matching position_label).

        Applies ``self._page_offset`` (printed page = PDF index - offset) so
        that typing a real/printed page number lands on the page that shows
        that printed page number on top of the window.
        """
        # Priority 1: Exact match with printed folio shown in position_label
        for i, page in enumerate(pages):
            if self.printed_folio_of(page) == number:
                return i

        # Priority 2: Clear printed folio near edges in OCR text
        if allow_heuristic:
            for i, page in enumerate(pages):
                try:
                    if _folio_near_edges(page_plain_text(page), number):
                        return i
                except Exception:  # noqa: BLE001
                    continue

        # Priority 3: Only when offset is 0, allow matching raw 1-based index
        if self._page_offset == 0 and 1 <= number <= len(pages):
            return number - 1
    ...
    def _folio_for_index(self, index: int) -> int:
        """Printed page number from label, else playlist index+1."""
        with self._lock:
            if 0 <= index < len(self._pages):
                folio = self.printed_folio_of(self._pages[index])
    ...
    def _printed_number_for(self, idx: int) -> int:
        """
        Best-effort printed page number for ``idx``: the OCR'd folio when
        readable (always correct when present), else an offset-adjusted
        PDF-index estimate — never a raw, un-adjusted PDF index. Ad pages
        frequently have no visible/readable page number, so this fallback
        matters a lot in practice for Ad-button captions.
        """
        if 0 <= idx < len(self._pages):
            folio = folio_of(self._pages[idx])
            if folio:
                return int(folio)
        return max(0, idx + 1 - self._page_offset)

    def current_skipped_ad(self) -> tuple[int, int] | None:
        """
        Return ``(page_index, folio_or_estimate)`` for Ad← UI, or None.
        ...
        with self._lock:
            ...
            if idx < 0 or idx >= len(self._pages):
                return None
            return (idx, self._printed_number_for(idx))
    ...
    def ad_queue(self) -> list[tuple[int, int]]:
        """Return ``[(page_index, folio_or_estimate), ...]`` for known ad pages."""
        with self._lock:
            out: list[tuple[int, int]] = []
            for idx in self._ad_queue:
                if 0 <= idx < len(self._pages):
                    out.append((idx, self._printed_number_for(idx)))
            return out
    ...
    def _notify_ad_skipped(self, ad_idx: int, cont_idx: int) -> None:
        with self._lock:
            ...
            folio = self._printed_number_for(ad_idx) if 0 <= ad_idx < len(self._pages) else 0
        ...
        for i in range(start, len(sentences)):
            with self._lock:
                ...
                if self._sent_idx != expected_prev and self._sent_idx != i:
                    return False
                self._sent_idx = i
            from ocr_read_aloud.text_clean import expand_contractions, sanitize_for_speech
            set_caption(sentences[i])
            speech_text = sanitize_for_speech(expand_contractions(sentences[i]))
