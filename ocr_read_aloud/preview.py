...
class ReadingPreview:
    ...
    def _build_window(self, master: Any, *, toplevel: bool) -> None:
        ...
        self._position = tk.Label(top, text="", anchor="w", font=("Segoe UI", 16, "bold"), fg="#ffffff", bg="#333333")
    ...
    def _set_photo(self, pil_image: Image.Image, *, store: bool = True) -> None:
        ...
        if store:
            ...
            # Do NOT reset _pan_x and _pan_y here — preserves manual pan/zoom during playback
    ...
    def show_region(
        ...
        *,
        ...
        # Skip highlight if the bbox covers the whole page — proof text has no
        # real per-line coordinates, so any drawn rect is a lie.
        covers_page = (
            width >= int(image.width * 0.95)
            and height >= int(image.height * 0.95)
        )
        if highlight and width > 0 and height > 0 and not covers_page:            
