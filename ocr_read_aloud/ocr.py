...
def _assign_columns(lines: list[OcrLine]) -> list[list[OcrLine]] | None:
    """
    Partition lines into 2+ macro columns when clear article gutters exist.
    Rejects gutters that are crossed by titles, box headers, or horizontal spans.
    """
    if len(lines) < 4:
        return None
    ...
    # Verify candidate gutters: if lines cross the gap (e.g. title spanning over table),
    # merge the two columns together so the table is read row-by-row.
    merged_ranges: list[tuple[float, float]] = [ranges[0]]
    for cur_lo, cur_hi in ranges[1:]:
        prev_lo, prev_hi = merged_ranges[-1]
        gutter_center = (prev_hi + cur_lo) / 2.0

        # Check if titles or lines bridge across this gutter
        has_crossing_line = any(
            (ln.left + 5 < gutter_center < ln.left + ln.width - 5)
            for ln in lines
        )
        # Also prevent micro-columns narrower than 14% page width from being isolated
        too_narrow = (cur_hi - cur_lo) < (0.14 * page_w)

        if has_crossing_line or too_narrow:
            # Merge with previous column
            merged_ranges[-1] = (prev_lo, max(prev_hi, cur_hi))
        else:
            merged_ranges.append((cur_lo, cur_hi))

    if len(merged_ranges) < 2:
        return None

    cols: list[list[OcrLine]] = [[] for _ in merged_ranges]
    for ln in lines:
        mid = _line_mid_x(ln)
        placed = False
        for i, (lo, hi) in enumerate(merged_ranges):
            if lo <= mid <= hi:
                cols[i].append(ln)
                placed = True
                break
        if not placed:
            dists = [abs(mid - (lo + hi) / 2.0) for lo, hi in merged_ranges]
...
def _is_column_rule(ln: OcrLine, *, col_w: float) -> bool:
    """True if line is a box border or divider within a column."""
    text = (ln.text or "").strip()
    if len(text) < 3:
        return False
    compact = "".join(c for c in text if not c.isspace())
    ...
    if rule_frac < 0.85:
        return False
    return ln.width >= 0.50 * max(col_w, 1.0)


def _sort_column_blocks_row_wise(col: list[OcrLine], *, med_h: float) -> list[OcrLine]:
    """
    Sort a column top-to-bottom, splitting at internal horizontal rules/boxes,
    and sorting each sub-block row-by-row (left-to-right within each line band).
    """
    if not col:
        return []

    c_lefts = [ln.left for ln in col]
    c_rights = [ln.left + ln.width for ln in col]
    col_w = float(max(c_rights) - min(c_lefts)) if c_rights and c_lefts else 1.0
    ...
    for ln in sorted(col, key=lambda x: x.top):
        if _is_column_rule(ln, col_w=col_w):
            if blocks[-1]:
                blocks.append([])
        else:
            blocks[-1].append(ln)
    ...
    for blk in blocks:
        if blk:
            out.extend(sorted(blk, key=row_key))
    return out


def _sort_lines_column_then_down(lines: list[OcrLine]) -> list[OcrLine]:
    """Title promote + column-then-down + row-wise table reading."""
    if not lines:
        return []
    ...
    if not columns:
        ...
        ordered_body = []
        for col in columns:
            ordered_body.extend(_sort_column_blocks_row_wise(col, med_h=med_h))
