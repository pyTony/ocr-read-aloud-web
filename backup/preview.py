...
class ReadingPreview:
    ...
    def _build_window(self, master: Any, *, toplevel: bool) -> None:
        ...
        self._merge_debug.pack(side="top", fill="x", padx=8, pady=(0, 2))
        ...
        if cmd == "image":
            ...
            if self._caption is not None:
                try:
                    ...
                    pass
        elif cmd == "merge_debug":
            if self._merge_debug is not None:
                try:
                    self._merge_debug.configure(text=(payload or "")[:800])
                except Exception:  # noqa: BLE001
                    pass
        elif cmd == "position":
    ...
    def _set_photo(self, pil_image: Image.Image, *, store: bool = True) -> None:
        ...
        # Fit (contain) with upscale allowed so the page fills the pane.
        try:
            from PIL import ImageOps

            fitted = ImageOps.contain(img, (tw, th), method=resample)
        except Exception:  # noqa: BLE001
            iw, ih = img.size
            if iw <= 0 or ih <= 0:
                return
        ...
        photo = self._ImageTk.PhotoImage(fitted)
        self._photo = photo
        try:
    ...
    def set_merge_debug(self, text: str) -> None:
        self._enqueue("merge_debug", text or "")
