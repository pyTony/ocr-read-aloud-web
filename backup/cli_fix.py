...
def run(args: argparse.Namespace) -> int:
    ...
    # 1. Resolve the path immediately and check existence
    input_path = Path(args.path).expanduser().resolve()
    
    if not input_path.exists():
        print(f"Error: Path not found: {input_path}", file=sys.stderr)
        print("Please provide a valid, absolute path to the PDF or image.", file=sys.stderr)
        return 1
    
    path = input_path # Use the validated absolute path
