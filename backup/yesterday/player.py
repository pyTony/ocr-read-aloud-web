...
class PageUnit:
    ...
    def speakable(self) -> list[OcrLine]:
        """Speak units: merged OCR line chunks (paragraph-sized), not sentences.

        Within a chunk, TTS speaks one sentence at a time so the top caption
        stays lockstep with ``_sent_idx`` (no wall-clock caption timer).
        """
        return _merge_lines_to_chunks(self.lines)
