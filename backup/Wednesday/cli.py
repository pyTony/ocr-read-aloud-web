...
def _vision_hint_for_page(page, *, model: str, host: str, page_offset: int = 0) -> str:
    ...
    raw_len = len(raw) if raw else 1

    is_noise = (
        len(words) < 15
        or (len(words) > 0 and (tiny_words / len(words)) > 0.50)
        or (raw_len > 0 and (alphanumeric_chars / raw_len) < 0.50)
    )

    # Calculate the *real* printed page number (folio) from the offset
    import re
    ...
    # ONLY trigger vision and print debug info when the page is actually noisy
    if is_noise:
        print(
            f"  [vision] Triggering vision for page {printed_folio} (PDF {pdf_index}) | "
            f"is_noise=True, words={len(words)}, tiny={tiny_words}, alpha_ratio={alphanumeric_chars/raw_len:.2f}",
            flush=True
        )
        ...
        try:
            ...
            if res and res.strip():
                print(f"  [vision] got result length {len(res)}", flush=True)
                return res.strip()
            else:
                print("  [vision] vision returned empty", flush=True)
    ...
    *,
    ...
    for page in pages:
        if replace_existing or page.label not in sections:
            # FIX: use page.lines, not page itself
...
def _run_speak_interleaved(
    *,
    ...
    # Create a temporary list with just the first page(s) to detect the offset
    temp_pages = []
    for label, image, lines in iter_ocr_pages(
        path,
        lang=lang,
        start_page=args.start_page,            # Start at PDF 14
        end_page=args.start_page + 2,          # Scan up to PDF 16 (just in case)
        dpi=args.dpi,
    ):
        temp_pages.append(PageUnit(label=label, image=image, lines=list(lines)))
        if len(temp_pages) >= 1:               # We only need the first page
            break

    # Compute offset immediately using only that first page
    computed_offset = args.page_offset if args.page_offset > 0 else compute_folio_offset(temp_pages)
    if computed_offset != 0:
        # Adjust the starting page NOW, before the real loader starts
    ...
    def loader() -> None:
        ...
        global_offset_initialized = False
        ...
        try:
            for label, image, lines in iter_ocr_pages(
                ...
                with pages_lock:
                    ...
                    # Compute offset exactly once when enough pages are present
                    if not global_offset_initialized:
                        from ocr_read_aloud.continue_links import compute_folio_offset
                        computed = compute_folio_offset(pages)
                        if computed != 0:
                            global_offset = args.page_offset if args.page_offset > 0 else computed
    ...
    # --- NOW create the controller (after page0 and preview exist) ---
    ...
    # 1. SET UP THE PREVIEW IMMEDIATELY (so it shows the page instantly)
    print("\n[Speaking — use preview controls or media keys]")
    active_preview = preview if (preview and preview.enabled) else None

    # 2. CREATE THE CONTROLLER WITH THE PREVIEW
    controller = PlaybackController(
        [page0],
        speaker,
        preview=active_preview,
    ...
    # 3. CONNECT THE PREVIEW TO THE CONTROLLER (starts rendering the page)
    ...
    # 4. NOW WAIT FOR PROOFREADING (UI is already visible, speaking waits)
