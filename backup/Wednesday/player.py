...
class PageUnit:
    ...
    def speakable(self) -> list[OcrLine]:
        ...
        # If this is Page 1 and the OCR text is just noise/garbage,
        # extract metadata dynamically from the filename/source path.
        if not chunks or (len(chunks) == 1 and len(chunks[0].text.strip()) <= 3):
            intro_text = "BYTE Magazine cover."
            source_str = str(getattr(self, "_source_path", "") or self.label)
            
            # Match patterns like YYYY_MM or YYYY-MM in the filename
            match = re.search(r"(19\d{2})[_-](\d{2})", source_str)
            if match:
                ...
                if month_name:
                    intro_text = f"BYTE Magazine, {month_name} {year}."
            
            return [OcrLine(text=intro_text, left=0, top=0, width=100, height=40)]
...
class PlaybackController:
    ...
    def __init__(
        ...
        *,
        ...
        skip_ad_pages: bool = True,           # <-- ADD THIS
        page_offset: int = 0,
        **kwargs,                              # <-- ADD THIS
        ) -> None:
    ...
    def position_label(self) -> str:
        ...
        nlines = max(1, len(page.speakable))
        parts = [page.label, f"para {li + 1}/{nlines}"]
        if len(sents) > 1:
            parts.append(f"sent {min(si, len(sents) - 1) + 1}/{len(sents)}")
    ...
    def _line_text(self, line: object) -> str:
            """Safe text extraction from an OcrLine or plain string."""
            if isinstance(line, str):
                return line.strip()
            return getattr(line, "text", "").strip()
         

    def _prepare_line_for_speech(self, text: str) -> str:
        from ocr_read_aloud.text_clean import expand_form_blanks, dehyphenate_inline, merge_short_fragments
        t = expand_form_blanks((text or "").strip())
        t = dehyphenate_inline(t)
        t = merge_short_fragments(t)          # <-- ADD THIS
        if not t:
            return ""
        if t.endswith(("-", "---", "--")):
            return t
    ...
    def _jump_to_continue(
        ...
        *,
        ...
        landing = find_continue_landing(
            ...
            offset=self._page_offset,   # <-- ADD THIS
        )
    ...
    def _try_ad_skip_from(self, left_page_idx: int) -> tuple[int, int] | None:
        ...
        with self._lock:
            ...
            if skip is not None:
                ...
                self._line_idx = 0
                self._sent_idx = 0
        self._enqueue_ad(ad_idx)
        self._notify_ad_skipped(ad_idx, cont_idx)
    ...
    def run(self) -> None:
        ...
        try:
            while True:
                ...
                completed = False
                try:
                    line_str = self._line_text(line)                       # safe text extraction
                    speech_text = self._prepare_line_for_speech(line_str)
                    completed = self._speak_chunk_with_caption(speech_text)
                except Exception as exc:  # noqa: BLE001
    ...
    def _speak_chunk_with_caption(self, text: str) -> bool:
        ...
        """
        from ocr_read_aloud.text_clean import expand_form_blanks, dehyphenate_inline
        full = expand_form_blanks((text or "").strip())
        full = dehyphenate_inline(full)
        if not full:
            return True
