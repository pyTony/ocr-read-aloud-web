...
def find_ad_skip(
    ...
    *,
    ...
    """
    If the next page is a skippable ad insert and the page after continues the article,
    return ``(ad_index, continue_index, continue_line_index)``.
    Now only skips if there is a valid article before and after (i.e., not at beginning).
    """
    ...
    # --- Skip if current page is not article-like (e.g., TOC, cover) ---
    if not looks_like_article_body(pages[current_idx]):
        return None

    # --- Existing checks: continued-from cues ---
    ...
    cont_f = folio_of(cont_page)
    linked_to_source = (
        (source_folio is not None and source_folio in cont_from)
        or (cont_f is not None and cont_f in source_on)
    )
    if not linked_to_source:
        return None

    # --- Only skip if middle page is truly an ad and next page is article ---
    mid_ad = (
    ...
    if not mid_ad or not cont_ok:
        return None

    # --- Additional: Check that the ad is between two article pages ---
    # We already have linked_to_source, so the current page is part of the article.
    # Ensure the ad page is not at the very beginning of the document (no previous article).
    # current_idx should already be > 0, but we can be safe.
    if current_idx == 0:
        return None

    # Also ensure the ad page doesn't look like a TOC or masthead.
    # If it looks like article body (high prose score), don't skip.
    if looks_like_article_body(ad_page):
        return None
